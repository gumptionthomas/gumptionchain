"""Demonstration tests for the 2026-06-02 signing_key/crypto threat-model audit.
...
def test_wc1_bespoke_encrypt_decrypt_removed():
    """WC1 (Low) — REMEDIATED. RSA-OAEP + AES-GCM hybrid `SigningKey.encrypt`
    / `SigningKey.decrypt` had no production caller after PR #111 replaced the
    ...
    """
    assert not hasattr(SigningKey, 'encrypt')
    assert not hasattr(SigningKey, 'decrypt')


def test_wc2_import_rejects_degenerate_exponent():
    """WC2 (Low) — REMEDIATED. `SigningKey.__init__` previously checked
