...
clients/sdk into the served static dir. Skips *.test.mjs and *-cli.mjs
(dev-only).

Usage: uv run python scripts/sync_sdk.py [--source .]
"""
...
DEST = Path(__file__).resolve().parent.parent / 'src/gumptionchain/static/sdk'
...
def main() -> None:
    ...
    src = Path(args.source).resolve() / 'clients/sdk'
    if not src.is_dir():
