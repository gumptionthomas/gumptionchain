...
`API access`_ to a node is granted by that node's operator. Once your signing_key address is on a node's role allowlist (see below to request access), configure your instance to use that node as a peer. Replace ``GCYourSigningKeyAddressGC`` with your signing_key address, ``peer.example.com`` with the host of the node you've been granted access to, and ``/path/to/signing_keys`` with the path to a directory containing your key (`PEM`_) file:
