// DEK-wrapping multi-method keyring for the persistent browser signing_key. ONE
// persisted signing_key, unlockable by EITHER a passphrase OR a passkey — the same
// signing_key, either method. Composes the existing primitives; no crypto is
// duplicated.
//
// Scheme:
...
//   { version, address, signing_key_ct: {iv, ciphertext},
...
// signing_key. A wrong passphrase rejects before any record mutation (fails closed).
...
// signing_key would become permanently unreachable) or a method that isn't enrolled.
