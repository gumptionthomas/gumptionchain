...
// A fake signing_key is just an opaque token here — the session never inspects it.
const fakeSigningKey = (id = 'w') => ({ id });

// --- the holder: set/get/lock/isUnlocked + onLock callbacks --------------

test('setSigningKey/getSigningKey/isUnlocked track the held reference', () => {
  const s = makeSession();
  assert.equal(s.isUnlocked(), false);
  assert.equal(s.getSigningKey(), null);
  const w = fakeSigningKey();
  s.setSigningKey(w);
  assert.equal(s.isUnlocked(), true);
  assert.equal(s.getSigningKey(), w);
});

test('lock() drops the signing_key reference and fires onLock callbacks', () => {
  const s = makeSession();
  let fired = 0;
  s.onLock(() => {
    fired += 1;
  });
  s.setSigningKey(fakeSigningKey());
  s.lock();
  assert.equal(s.getSigningKey(), null);
  assert.equal(s.isUnlocked(), false);
...
test('re-unlock re-arms the idle timer after a prior auto-lock', () => {
  ...
  // fire (auto-lock), then setSigningKey again (a re-unlock) must re-arm exactly
  ...
  // Re-unlock: setSigningKey must re-arm the (still-configured) idle timer.
  s.setSigningKey(fakeSigningKey());
  assert.equal(s.isUnlocked(), true);
  assert.equal(timer.pending().length, 1);
  timer.fireAll();
