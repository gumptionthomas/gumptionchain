...
4. **Reload, then unlock.** Reload the page (proving the key is recovered from
   ...
   provider this is a single confirm. Confirm the displayed `gc1…` address
   matches the one from step 3 and that a base64 signature is produced.
...
The backup crypto (`gc-backup.mjs` — PBKDF2-SHA256 → AES-GCM-256, plus the raw
`gcsec1…` secret path) is fully covered by `node --test`. Only the browser glue
in the demo page (file download/upload, passphrase prompt, clipboard) is manual:
...
- The passkey credential is a **separate** WebAuthn credential (ES256/RS256,
  ...
  signing_key's Ed25519 signing key — that key is generated by
  `SigningKey.generate()` and stored AES-GCM-encrypted under a key derived from
  the PRF output.
