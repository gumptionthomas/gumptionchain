...
// Also: social-platform binding envelopes (signing_key ↔ platform identity, #251).
...
export async function signStakeAttestation(signing_key, claim, { timestamp } = {}) {
  return signMessage(signing_key, buildStakeMessage(claim), { timestamp });
}
...
export async function signSocialBinding(signing_key, claim, { timestamp } = {}) {
  return signMessage(signing_key, buildBindingMessage(claim), { timestamp });
}
