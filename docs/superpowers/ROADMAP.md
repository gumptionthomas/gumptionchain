...
- **A4.a (High) — validate `*_ADDRESSES` regexes at config load.** Reject overbroad/unanchored patterns (e.g. `CC.*CC`) that escalate every authenticated address. Highest priority — config-triggered; needs only an ordinary authenticated address (any wallet that completes the handshake), no `SECRET_KEY`/server compromise. Test: `test_a4_a_overbroad_admin_regex_escalates_reader`.
