...
- ✅ **N2 (Medium) — mempool has no admission cap** — `pending_txns` admits unbounded distinct valid txns (`node.py:95-102`); every expiry/mill/pending scan re-materializes the whole pool (O(mempool)). Fix: `CC_MAX_PENDING_TXNS` cap + indexed/SQL-filtered expiry. Test: `test_n2_mempool_has_no_admission_cap`. Closed by PR #PRNUM.
