...
Two open findings from the 2026-05-29 verification pipeline audit (A2.e, A4.c, A7.b, and A7.h are closed; see Closed items). Each has a demonstration `@pytest.mark.xfail(strict=True)` test in `tests/test_verification_audit.py`; remediation removes the xfail and the test becomes a real pass (strict mode forces this).
