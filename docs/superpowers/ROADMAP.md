...
The 2026-05-31 API authentication audit ([report](audits/2026-05-31-api-authentication-audit.md); design+plan [#101](https://github.com/gumptionthomas/cancelchain/pull/101), findings+tests [#102](https://github.com/gumptionthomas/cancelchain/pull/102)) produced **8 findings: 0 Critical / 0 High (A4.a remediated, PR #105) / 2 Medium (A3.a + A5.b remediated, PR #107; A3.b remediated, PR #PRNUM) / 2 Low**, each with a `@pytest.mark.xfail(strict=True)` demonstration test in `tests/test_auth_audit.py`. Each remediation flips its xfail to a real pass. Grouped by shared fix (see the audit's Recommendations for full detail):
