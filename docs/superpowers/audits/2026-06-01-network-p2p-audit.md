...
**0 Critical / 0 High / 2 Medium / 1 Low.** The trusted-input boundary holds: every confirmed finding is an **availability / resource-bounding gap on an otherwise-validated, otherwise-authorized path** — "valid, authenticated peer input in hostile volume, depth, or pattern," exactly the in-scope case. None reduces to a verification-validity or authorization break. N1 (the sole High) is **remediated** — `fill_chain`'s ancestor walk is now capped at `MAX_CHAIN_FILL_DEPTH` and `request_block` verifies the returned block's hash equals the requested hash before staging. The recurring root was a **missing-bound pattern**: the node had no depth cap on `fill_chain`'s ancestor walk, no size cap on the `pending_txns` mempool, and no idempotency/rate guard on duplicate transaction gossip. A secondary theme: **validation is deferred past the point where resources are already committed** — mempool admission defers chain/UTXO checks to mill time, so an attacker pays ~zero to make the victim commit unbounded pending rows.

| ID | Adversary | Severity | Description | Status | Demonstration test |
|---|---|---|---|---|---|
| N1 | Resource-exhaustion / eclipse / framing (merged) | **High** | `fill_chain` stages an attacker-controlled, uncapped chain of ancestor blocks — one HTTP round-trip + one committed staging row per ancestor, no depth bound, never terminates | ✅ remediated | `test_network_audit.py::test_n1_fill_chain_has_no_depth_cap` |
...
1. **Missing-bound pattern repeats across the P2P/staging/mempool surface.** As originally found: no depth cap on `fill_chain`'s ancestor walk (N1), no size cap on the `pending_txns` mempool (N2), no idempotency/rate guard on duplicate txn gossip (N3). Every confirmed finding is a "valid authenticated input in hostile volume/pattern" availability issue — the trusted-input boundary holds; the **resource-bounding boundary does not**. (N1's `fill_chain` cap has since landed; the unbounded mempool and gossip paths remain.)

2. **Validation is deferred past the point where resources are already committed.** As originally found: `request_block` ran only structural Pydantic validation and never checked returned-hash == requested-prev_hash; full `Block.validate()` (PoW/merkle/chain) runs only in the apply phase after the entire chain is staged. Mempool admission checks shape+signature+txid but defers chain/UTXO/balance/double-spend to mill time. In both paths an attacker paid ~zero to make the victim commit unbounded staging/pending rows before any meaningful rejection. Pulling a cheap structural+identity check forward (and capping the work) closes both. (N1's `request_block` hash check + `fill_chain` cap have since landed; the mempool-admission half remains in N2.)
