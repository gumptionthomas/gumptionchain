...
**0 Critical / 0 High / 0 Medium / 2 Low** (at audit time). **Both findings (WEB1, WEB2) are now remediated — audit fully closed; every demonstration in `tests/test_web_audit.py` is a passing regression.**
...
|---|---|---|---|---|---|
| WEB1 | remote visitor / clickjacker / MIME-sniffer | Low | At audit time, no `Content-Security-Policy`, `X-Frame-Options`, `X-Content-Type-Options`, `Referrer-Policy`, or HSTS shipped on HTML responses — `init_app` wired no `after_request`/Talisman; the only header mutation was `Content-Type: application/json` on the JSON API. Defense-in-depth gap; held at Low because the explorer is read-only/cookieless/login-less (no session to steal, no action to forge) and there is no confirmed XSS for a missing CSP to backstop. | ✅ Remediated — an `@app.after_request` hook (`set_security_headers`) sets all five on every response (HSTS HTTPS-only); `test_web1_security_headers_present` is now a passing regression. | `test_web1_security_headers_present` (regression) |
| WEB2 | remote visitor triggering a view exception | Low | At audit time, all four views' generic handler (`index_view`/`chains_view`/`block_view`/`transaction_view`) ended `except Exception as e: current_app.logger.exception(e); return e`. An `Exception` is not a valid Flask return — `make_response` raises `TypeError`, resolved to a generic 500. Residue: a latent response-correctness bug + a full traceback written to the app log on every view error; a direct traceback-to-client would additionally require `DEBUG=True` (not shipped). | ✅ Remediated — the generic handler now logs the traceback and `abort(500)`s (returns a controlled 500, not a raw `Exception`), keeping `except HTTPException: return e`; `test_web2_view_error_yields_clean_500` is now a passing regression. | `test_web2_view_error_yields_clean_500` (regression) |
