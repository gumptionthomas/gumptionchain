...
This audit traced 42 attack attempts across 7 adversary categories through cancelchain's verification pipeline (`Node.receive_transaction`, `Node.receive_block`, `Block.validate`, `Chain.validate_block`, `Chain.validate_block_txn`, `Chain.validate_block_coinbase`, and the surrounding fill / reorg / pending-pool machinery). Six findings were originally confirmed (all Medium or Low; no Critical or High). Four have since been remediated (A2.e, A4.c, A7.b, A7.h); two remain open (A7.e, A1.f). Each open finding is paired with a `@pytest.mark.xfail(strict=True)` demonstration in `tests/test_verification_audit.py`.
...
2 open findings: 0 Critical / 0 High / 0 Medium / 2 Low (post-A7.h). Sorted by severity (highest first), then by ID within each severity.
