...
This audit traced 42 attack attempts across 7 adversary categories through cancelchain's verification pipeline (`Node.receive_transaction`, `Node.receive_block`, `Block.validate`, `Chain.validate_block`, `Chain.validate_block_txn`, `Chain.validate_block_coinbase`, and the surrounding fill / reorg / pending-pool machinery). Six findings were originally confirmed (all Medium or Low; no Critical or High). **All six have since been remediated** (A2.e, A4.c, A7.b, A7.h, A7.e, A1.f); none remain open. Each was paired with a `@pytest.mark.xfail(strict=True)` demonstration in `tests/test_verification_audit.py`, now converted to a passing regression test.
...
The findings concentrated in two structurally related pockets, both now remediated. First, **operational state management around `Node.fill_chain` / `ChainFill`** (A2.e Medium): the apply loop committed each block individually, so a hostile peer could force partial adoption of a fork prefix by appending a cheap-to-construct invalid tip — closed by the deferred-commit / single-rollback fix. Second, **accounting-side coinbase replay** (A4.c Medium): a malicious miller could replay another miller's coinbase txn to inflate the original miller's wallet-balance reads — closed by binding the block's `prev_hash` into the coinbase txid (so legitimate consecutive coinbases are unique) plus a local `cb.prev_hash == block.prev_hash` binding check raising `MismatchedCoinbaseError`. The fourth Low finding in that area was the pending-pool replay **A1.f**: a transactor could replay a mined txid back into the pending pool until 4h `TXN_TIMEOUT` expiry — closed by a global `TransactionDAO.get` admission check in `Node.receive_transaction` raising `DuplicateMinedTransactionError`. Three further Low findings on Adversary 7 surfaced conceptual gaps (missing checks) rather than off-by-one errors, all since remediated: alternate-genesis fragments the chain registry (A7.b), `TXN_TIMEOUT` used three different comparison operators across three call sites (A7.e), and subjects accepted arbitrary UTF-8 codepoints including control characters and bidi overrides (A7.h).
...
**Outcome:** REJECTED (post-remediation) — the coinbase is intrinsically bound to the block it rewards. `Transaction` carries a `prev_hash` field that is folded into the coinbase's hashed `data_csv` (hence its txid); `Block.create_coinbase` sets it to the block's parent hash. T_cb was minted for B_orig, so it carries B_orig's parent hash; B_adv extends the chain tip, so `B_adv.prev_hash` is B_orig's *own* hash. `Chain.validate_block_coinbase` (in `src/cancelchain/chain.py`) now compares `cb.prev_hash != block.prev_hash` and raises `MismatchedCoinbaseError` (a subclass of `InvalidCoinbaseError`) on the mismatch — so B_adv is rejected in `validate_block` and never persists. The check is purely local (no lineage walk, no `self.last_block` dependence), so it behaves identically in the add-block path and in `Chain.validate()` full-chain revalidation. Binding `prev_hash` into the txid also makes the two legitimate consecutive coinbases distinct, so the duplicate-m2m / balance-inflation surface is closed at its root: no two legitimately-mined blocks share a coinbase txid.

**Historical note (pre-fix behavior).** Before remediation, the coinbase txid carried no block identity. `Chain.validate_block_coinbase` enforced only the REWARD amount and S/G/M shape, so a verbatim replay of T_cb as B_adv's coinbase was ACCEPTED: `Transaction.to_dao()` returned the existing `TransactionDAO` row for the already-persisted txid, appending a second `block_transactions` m2m row associating T_cb with B_adv. T_cb then appeared in two longest-chain blocks, so `BlockDAO.longest_chain_transactions_q`'s join double-counted its REWARD outflow and `ChainDAO.wallet_balance` inflated M_orig's reported balance by one REWARD per replay (the `InflowDAO` `UniqueConstraint('txid', 'idx')` kept the inflated balance unspendable, but the accounting layer reported the wrong number). The v1 remediation (a chain-lineage coinbase-txid uniqueness check) proved unimplementable — coinbase txids are not unique across legitimately-mined same-second blocks — and was superseded by the v2 `prev_hash` binding described above.

**Result:** Validation correctly rejects. No finding.

**Demonstration tests:** `test_a4_c_ii_coinbase_replay_inflates_balance` (the replay is rejected with `MismatchedCoinbaseError`) and `test_a4_c_coinbase_block_binding` (two consecutive legitimate blocks have distinct coinbase txids, and a mismatched binding is rejected) in `tests/test_verification_audit.py`.
...
**Outcome:** REJECTED (post-remediation) — a second genesis is now refused. `is_genesis_block(block)` is still only a `prev_hash == GENESIS_HASH` flag, but `Chain.validate_block` now adds a canonical-genesis check that raises `DuplicateGenesisError` when a *different* genesis is already persisted, so the registry can no longer be fragmented. (Pre-remediation, the pipeline accepted arbitrarily many "genesis" blocks — each spawning a new sibling chain — because no "is the canonical genesis already taken?" predicate existed.)

✅ **Remediated.** **Finding A7.b — Severity Low:** `Chain.validate_block` (`src/cancelchain/chain.py:170-198`) accepts any block whose `prev_hash == GENESIS_HASH`, `idx == 0`, and `target == MAX_TARGET`, regardless of whether a different genesis block is already persisted. Each accepted alternate-genesis creates a fresh `ChainDAO` row (via `Node.add_block`'s `create_chain` fallback at `src/cancelchain/node.py:187`), fragmenting the chain registry into N parallel single-block chains. `ChainDAO.longest()` still picks the canonical genesis via deterministic tiebreaker, so chain-correctness is preserved — but the DB accumulates one unrooted `ChainDAO` (and one `BlockDAO`/`TransactionDAO`/`OutflowDAO`) row per submission. A MILLER-role adversary can therefore inflate the chain registry with cheap-to-mill alternate genesis blocks (production `MAX_TARGET = 6 leading hex zeros` is still tractable for any modest hashrate). The attack does not break value conservation (each genesis pays `REWARD` to its own miller, and only the canonical genesis's `LongestChainBlockDAO` rows feed into `wallet_balance` reads), but it is a DB-bloat / inventory-pollution gap with no operational recovery path. Remediated: `Chain.validate_block` now calls `Block.genesis_from_db()` and raises `DuplicateGenesisError(InvalidBlockError)` when a different genesis is already persisted; the helper keys on `idx == 0` to avoid a `chain.py → block.py` circular import. Regression tests: `test_a7_b_alternate_genesis_fragments_chain_registry` and `test_a7_j_disjoint_genesis_reorg_rejected`.

**Remediation sketch:** In `Chain.validate_block` (`src/cancelchain/chain.py:170`), after the `is_genesis_block(block)` branch passes the parent / idx / target checks, look up whether a genesis block is already persisted: `existing_genesis = db.session.execute(db.select(BlockDAO).where(BlockDAO.idx == 0, BlockDAO.prev_hash == GENESIS_HASH)).scalar_one_or_none()`. If `existing_genesis is not None` and `existing_genesis.block_hash != block.block_hash`, raise a new `DuplicateGenesisError(InvalidBlockError)` exception. This enforces canonical-genesis uniqueness at the validation layer without changing the `is_genesis_block` predicate or breaking the legitimate first-genesis flow. (As implemented, this was factored into a `Block.genesis_from_db()` helper keyed on `idx == 0` only — `idx == 0 ⟺ genesis`, and an `idx`-only key lets `block.py` avoid a circular import on `GENESIS_HASH`. The check is gated on `is_genesis_block(block) and block.idx == 0` and runs *before* the parent/target checks, so a block with `prev_hash == GENESIS_HASH` but `idx != 0` is NOT treated as a duplicate genesis — it falls through to the precise `InvalidBlockIndexError`. A genuine second genesis with a wrong target is rejected as `DuplicateGenesisError` rather than `InvalidTargetError`; both are `InvalidBlockError` subclasses.) An equally good alternative is to make `GENESIS_HASH` resolve to a hardcoded canonical block (committed as part of `db.create_all()` / migrations); but that requires migration work and changes the bootstrap flow, so the validate-time uniqueness check is the more conservative fix.
...
**Outcome:** REJECTED (post-remediation). `validate_subject` / `validate_raw_subject` now require the decoded raw subject to satisfy `str.isprintable()`, so control characters, bidi overrides, and zero-width characters are refused. (Pre-remediation, the pipeline enforced length but not character class.)

✅ **Remediated.** **Finding A7.h — Severity Low** (pre-remediation behavior described below): `validate_subject` (`src/cancelchain/payload.py:39-46`) and `validate_raw_subject` (line 49-55) enforced only length bounds (`1 <= len <= 79`) and canonical base64-url encoding round-trip; they accepted any UTF-8 codepoint including null bytes (`\x00`), C0/C1 control characters (`\x07` BEL, `\x1b` ESC, `\x0a` LF, `\x7f` DEL), bidirectional override (`‮` RLO), zero-width joiners (`‍`), and zero-width spaces. Subjects flow through `OutflowDAO.subject` into multiple read paths: `BalanceView` rendering (`src/cancelchain/api.py`), CLI `subject` commands (`src/cancelchain/command.py`), `wallet_leaderboard` JSON responses. A subject like `f'spoofed{chr(0x1b)}[31mRED'` or `'‮redips'` would render correctly per byte but display deceptively in any terminal/HTML consumer that doesn't strip control characters. No value-conservation invariant is violated (the malicious subject is still distinct from the spoofed one at the byte level), but the chain commits to a string the application layer is unlikely to handle safely. Remediated: a `_valid_raw_subject()` helper in `src/cancelchain/payload.py` now requires `str.isprintable()` (in addition to the length bound), and both `validate_subject` (after `decode_subject`) and `validate_raw_subject` route through it. Regression: `test_a7_h_non_printable_subject_accepted` plus unit tests in `tests/test_payload.py`.

**Remediation sketch:** Add a content-class check in `validate_raw_subject` (`src/cancelchain/payload.py:49-55`) after the length check: e.g., `if not all(_is_safe_codepoint(c) for c in raw_subject): return False`. The safe-codepoint predicate should reject Unicode general categories `Cc` (control), `Cf` (format — includes bidi overrides and zero-width chars), `Cn` (unassigned), and `Cs` (surrogates). The `unicodedata` stdlib gives `unicodedata.category(c)` for this. The check needs to be applied symmetrically in both `validate_raw_subject` (raw form, used by CLI input) and inside `validate_subject` after `decode_subject` (encoded form, used by API/JSON input). Adding the check tightens the surface without breaking any reasonable real-world subject (which would be human-readable text); existing tests use ASCII strings like `'failing tests'` / `'bugs'` / `'vogons'`, all of which pass the category check. (As implemented: superseded by a single `str.isprintable()` check in a shared `_valid_raw_subject()` helper that both validators call — equivalent in intent but stricter (it also rejects private-use `Co` and non-ASCII whitespace) and simpler than a per-category `unicodedata.category()` loop.)
...
### 2. A4.c (Medium) — ✅ Implemented — coinbase-to-block binding in `Chain.validate_block_coinbase`

✅ **Implemented (v2 binding fix).** `Transaction` now carries a `prev_hash` field that is folded into the coinbase's hashed `data_csv` (hence its txid); `Block.create_coinbase` sets it to the block's parent hash (`src/cancelchain/block.py`). `Chain.validate_block_coinbase` (in `src/cancelchain/chain.py`) compares `cb.prev_hash != block.prev_hash` and raises `MismatchedCoinbaseError(InvalidCoinbaseError)` (`src/cancelchain/exceptions.py`) on the mismatch. Binding `prev_hash` into the txid makes legitimately-mined consecutive coinbases distinct (closing the duplicate-m2m / balance-inflation surface at its root); the local binding check rejects a coinbase replayed onto a different-parent block. The check is purely local — no lineage walk, no `self.last_block` dependence — so it is correct in both the add-block path and `Chain.validate()` full-chain revalidation. A nullable `TransactionDAO.prev_hash` column persists the binding (regenerated base migration, pre-1.0). The v1 plan's lineage coinbase-txid uniqueness check proved unimplementable (coinbase txids are not unique across legitimate same-second blocks) and was superseded by this binding approach. Blast radius: closes the wallet-balance inflation surface for any MILLER-role adversary; restores the no-double-counting invariant for coinbase outflows. Acceptance signal: `test_a4_c_ii_coinbase_replay_inflates_balance` is now a passing regression test (xfail removed); `test_a4_c_coinbase_block_binding` asserts the binding invariant.

### 3. A7.b (Low) — ✅ Implemented — canonical-genesis check in `Chain.validate_block`

✅ **Implemented.** The fix lives in `Chain.validate_block` (`src/cancelchain/chain.py`). In the genesis branch — gated on `is_genesis_block(block) and block.idx == 0`, placed early, right after the `FutureBlockError` check and *before* the parent / target checks — it calls the new `Block.genesis_from_db()` helper (`src/cancelchain/block.py`) and raises `DuplicateGenesisError(InvalidBlockError)` when an existing genesis is found whose `block_hash` differs from the candidate. `Block.genesis_from_db()` reuses `BlockDAO.get(idx=0)`; it keys on `idx == 0` rather than `prev_hash == GENESIS_HASH` (the two are equivalent since `idx == 0 ⟺ genesis`) specifically so `block.py` need not import `GENESIS_HASH` from `chain.py` — that would be a circular import. The `block_hash`-equality guard keeps the check idempotent, so it is a no-op for the first genesis (empty DB) and for the canonical genesis during `Chain.validate()` full-chain revalidation. Blast radius: closes the chain-registry fragmentation surface from any MILLER-role adversary AND closes A7.j's only entry path (disjoint-ancestor reorg can only occur if an alternate genesis was first admitted). Acceptance signal: `test_a7_b_alternate_genesis_fragments_chain_registry` is now a passing regression test (xfail removed); `test_a7_j_disjoint_genesis_reorg_rejected` proves the A7.j reorg path is closed. Two-for-one fix.

### 4. A7.h (Low) — ✅ Implemented — printable-subject check in `validate_subject` / `validate_raw_subject`

✅ **Implemented.** The fix lives in `src/cancelchain/payload.py`. A shared `_valid_raw_subject()` helper now requires `str.isprintable()` in addition to the existing length bound, and both `validate_subject` (after `decode_subject`) and `validate_raw_subject` route through it. The audit's design intent originally implied human-readable subjects; the audit just surfaced that no check existed to enforce it. Blast radius: closes the rendering-spoof surface for any consumer of `OutflowDAO.subject` (`BalanceView`, CLI `subject` commands, `wallet_leaderboard`). Acceptance signal: `test_a7_h_non_printable_subject_accepted` is now a passing regression test (xfail removed), with printability unit tests in `tests/test_payload.py`. Implemented via `str.isprintable()` rather than a per-category `unicodedata.category()` loop — stricter (also rejects private-use and non-ASCII whitespace) and simpler. Note: while the audit's Non-goals deferred "spec changes to validation rules", A7.h is arguably already in the spec's intent — the audit just confirms the check doesn't exist.

### 5. A7.e (Low) — ✅ Implemented — single `txn_is_expired()` definition

✅ **Implemented.** The expiry boundary is now defined once by a `txn_is_expired(txn_ts, reference_dt)` helper in `src/cancelchain/block.py` (expired ⟺ `txn_ts < reference_dt - TXN_TIMEOUT`; open boundary, so a txn exactly `TXN_TIMEOUT` old is alive). The three Python sites route through it: `Block.validate_transaction` (behavior-identical — the same strict `<`, now via the helper, so consensus is unchanged), `Node.discard_expired_pending_txns` (was `<=`), and `Miller.pending_chain_txns` (was `>`); each pending-pool site captures `reference_dt = now()` once before iterating, so expiry is order-independent. The `PendingTxnDAO.json_datas` SQL already matched (`timestamp >= cutoff`) and carries a cross-ref comment. Implemented as a shared helper rather than three inline operator swaps, so the boundary is defined once and cannot drift again. Blast radius: the only observable change is at the exact one-second boundary instant, where the pending pool now retains/selects a txn the block validator already treated as alive; no consensus change. Acceptance signal: `test_a7_e_txn_timeout_boundary_inconsistency` is now a passing regression test (xfail removed), plus `test_txn_is_expired_boundary`, `test_pending_chain_txns_boundary_alive`, and `test_pending_chain_txns_expired_excluded`.
