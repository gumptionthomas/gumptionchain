...
1. **XSS / content-injection** — script or markup execution via the attacker-authored rendered fields (the outflow `subject`/`forgive`/`support` and its `human_subject` decode; the transaction `address`/`public_key`/`signature`) or any other interpolation; escaping-context errors across EVERY context — HTML text, `<title>`, and the JS-attribute `onclick="…'{{ url_for(...) }}'…"` (`block.html:65`, `transaction.html:56`); any `| safe`/`Markup`; and whether autoescape is actually on for every rendered template.
