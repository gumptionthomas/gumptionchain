...
- **User-controlled `subject` rendered in `transaction.html`** (`{{ o.subject }}` / `{{ o.subject | human_subject }}`, lines 101-103) — the one attacker-authored field that reaches the DOM. (Jinja autoescape is on by default for `.html` and `human_subject` returns a plain `str`, so this is likely *defended* — the audit confirms, doesn't assume.)
