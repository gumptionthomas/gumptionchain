...
- **Modify:** `tests/test_verification_audit.py` — (1) removes the `@pytest.mark.xfail(strict=True)` decorator on `test_a4_c_ii_coinbase_replay_inflates_balance` (its body unchanged save a post-fix docstring refresh); (2) adds `test_a4_c_cross_fork_coinbase_replay_accepted`, a non-regression test that builds a competing sibling fork, replays a canonical-fork coinbase onto it, and asserts `validate_block_coinbase` does NOT raise `DuplicateCoinbaseError` (guarding the new check's primary edge case of over-rejecting cross-fork replay); (3) adds `Chain` and `DuplicateCoinbaseError` to the module imports; (4) updates the **module-level docstring**, which currently claims every test is `@pytest.mark.xfail(strict=True)` — stale since A2.e was remediated in PR #87 and further stale here (A4.c un-xfailed + a non-xfail cross-fork test added) — to describe the open-finding (xfail) vs remediated-finding (plain pass) vs non-regression states.
- **Modify:** `docs/superpowers/audits/2026-05-29-verification-pipeline-audit.md`:
  - Remove the A4.c row from the Findings table.
  - Replace the entire stale tail of §Adversary 4 → Attack c.ii — the contiguous run from `**Outcome:** ACCEPTED` through `**Demonstration test:**`, which includes the `**Consequence**` paragraph, the schema-layer paragraph, `**Finding A4.c**`, and the `**Remediation sketch:**` paragraph (the last two both carrying the rejected `start_block=block` + m2m self-exclusion guidance) — with a post-remediation block: a REJECTED outcome describing the shipped `start_block=parent` design, a one-paragraph historical record of the pre-fix gap, a "Result: validation correctly rejects" note, and the demonstration-test pointer. Replacing the whole run removes every `start_block=block` copy in this section in one edit.
