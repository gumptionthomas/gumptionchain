...
- **Single-block receive path** (`Node.receive_block` → `Node.process_block` → `Node.add_block`). Today this path runs two commits per block — one from `Block.to_db()` and one from `Chain.to_db()` (after `Chain.add_block`). The default `commit=True` preserves that two-commit behavior for all non-fill_chain callers; the existing `except SQLAlchemyError` handler still rolls back on a single-block failure. No behavior change for this path; the only new wrinkle is that callers who deliberately pass `commit=False` (only `fill_chain`) get a single batched commit at the end instead.
