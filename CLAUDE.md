...
- **Don't auto-merge — open the PR and wait for the author's review.** Green CI plus the subagent review make a PR *ready*, not *merged*; the user's own review/test is part of the bar and the PR is the natural checkpoint for it. So: open the PR, post a short "ready for review" summary (what changed + how to test), and **stop**. Merge only on the author's explicit signal — `mwg`, "merge it" / "lgtm" / "ship it", or they merge it themselves. Trivial dependabot bumps, or an explicit "merge as you go", can override this.
- **`mwg`** = "merge when green" — the author's explicit **merge signal**: `gh pr checks <N> --watch`, then squash-merge once green. Don't invoke this on your own initiative right after opening a PR — wait for the user to ask.
